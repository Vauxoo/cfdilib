...
class TestSignatory(unittest.TestCase):
    ...
    def test_001_managing_known_errors(self):
        ...
        signatory.sign('XML')
        self.assertTrue(signatory.message.find('unknown') >= 0,
    ...
    def test_002_managing_known_errors(self):
        ...
        signatory.sign('XML')
        print signatory.message
        self.assertTrue(signatory.message.find(url) >= 0,
                        'Passing an incorrect url the message was not the expected: %s' % signatory.message)

    def test_003_managing_known_errors(self):
        '''Given a well formed url but incorrect user password client is setted correctly.'''
        # TODO: Mock this test
        ...
        self.assertTrue(isinstance(signatory.client, suds.client.Client),
                        'Passing a correct  url the client object is not what I expected')
        self.assertTrue(signatory.message is None,
                        'The message on the invoice object should be False and it is not')
